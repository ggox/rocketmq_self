...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public CompletableFuture<RemotingCommand> asyncProcessRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            case RequestCode.CONSUMER_SEND_MSG_BACK: // 消费端消费情况返回消息
                // 消息重投处理
            ...
            default:
                ...
                // 构造消息链路跟踪相关的TraceContext
                mqtraceContext = buildMsgContext(ctx, requestHeader);
                // broker端回调sendMessageHook
    ...
    private CompletableFuture<RemotingCommand> asyncConsumerSendMsgBack(ChannelHandlerContext ctx,
        ...
        if (this.hasConsumeMessageHook() && !UtilAll.isBlank(requestHeader.getOriginMsgId())) { // 有钩子并且原始消息id不为空
            ConsumeMessageContext context = buildConsumeMessageContext(namespace, requestHeader, request);
            this.executeConsumeMessageHookAfter(context); // 回调钩子（after）
        }
        // 消费组订阅消息
        ...
        // 校验扩展信息中有没有设置原始消息的topic，没有就设置一下
        ...
        if (request.getVersion() >= MQVersion.Version.V3_4_9.ordinal()) { // 版本适配
            maxReconsumeTimes = requestHeader.getMaxReconsumeTimes();
        }

        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes 
            || delayLevel < 0) { // 重试次数用完了 or delayLevel < 0  直接投放到死信队列
            ...
            if (0 == delayLevel) { // 等于0 服务端控制：3 + 已经重新消费的次数
                delayLevel = 3 + msgExt.getReconsumeTimes();
            }
            msgExt.setDelayTimeLevel(delayLevel);
        }
        // 这里补充一下：重试消息一般都会设置一个延时时间，所以这个消息不会马上投放到重试队列，而是先投放到这个延时级别的队列上，等到了时间才会重新投放到重试队列上
        ...
        msgInner.setReconsumeTimes(msgExt.getReconsumeTimes() + 1); // 消费次数加一
    ...
    private CompletableFuture<RemotingCommand> asyncSendMessage(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        if (response.getCode() != -1) { // 表明已经存在异常了
            return CompletableFuture.completedFuture(response);
        }
        ...
        if (queueIdInt < 0) { // queueId小于0，随机选择一个queue
        ...
        } else { // 非事物消息，使用MessageStore异步存储消息
    ...
    private boolean handleRetryAndDLQ(SendMessageRequestHeader requestHeader, RemotingCommand response,
        ...
        if (null != newTopic && newTopic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {
            ...
            // 当前重试次数达到或者超越了最大重试次数就不在发送到重试队列而是死信队列了
    ...
    private RemotingCommand handlePutMessageResult(PutMessageResult putMessageResult, RemotingCommand response,
        ...
        switch (putMessageResult.getPutMessageStatus()) {
            ...
            case FLUSH_DISK_TIMEOUT: // 刷新磁盘超时
                ...
                sendOK = true;
                break;
            case FLUSH_SLAVE_TIMEOUT: // 属性Slave节点超时
                response.setCode(ResponseCode.FLUSH_SLAVE_TIMEOUT);
                sendOK = true;
                break;
            case SLAVE_NOT_AVAILABLE: // 不支持slave
                response.setCode(ResponseCode.SLAVE_NOT_AVAILABLE);
                sendOK = true;
                break;

            // Failed 以下是失败的情况
            case CREATE_MAPEDFILE_FAILED: // 创建内存映射文件失败
                response.setCode(ResponseCode.SYSTEM_ERROR);
                response.setRemark("create mapped file failed, server is busy or broken.");
                break;
            case MESSAGE_ILLEGAL: // 消息不合法
            case PROPERTIES_SIZE_EXCEEDED: // 消息大小超标
                response.setCode(ResponseCode.MESSAGE_ILLEGAL);
                response.setRemark(
                    "the message is illegal, maybe msg body or properties length not matched. msg body length limit 128k, msg properties length limit 32k.");
                break;
            case SERVICE_NOT_AVAILABLE: // 服务不可用，可能是磁盘不够用了或者内存太小了等
                response.setCode(ResponseCode.SERVICE_NOT_AVAILABLE);
                response.setRemark(
                    "service not available now, maybe disk full, " + diskUtil() + ", maybe your broker machine memory too small.");
                break;
            case OS_PAGECACHE_BUSY: // os PageCache 繁忙，暂时启动了流控
                response.setCode(ResponseCode.SYSTEM_ERROR);
                response.setRemark("[PC_SYNCHRONIZED]broker busy, start flow control for a while");
                break;
            case UNKNOWN_ERROR: // 位置错误
                response.setCode(ResponseCode.SYSTEM_ERROR);
