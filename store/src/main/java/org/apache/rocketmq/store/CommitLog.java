...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            // Began to recover from the last third file 从最新的三个文件开始恢复
            ...
            while (true) {
                // 从commitlog中抽取一个消息，和ReputMessageService不同的是校验CRC是通过配置文件配置的
                DispatchRequest dispatchRequest = this.checkMessageAndReturnSize(byteBuffer, checkCRCOnRecover);
                int size = dispatchRequest.getMsgSize();
                // Normal data 是否是正常消息，正常消息更新offset
                ...
                else if (dispatchRequest.isSuccess() && size == 0) { // 没有发生异常，但是size是0，应该是文件跑到最后了
                    index++; // 获取下一个文件
                ...
                else if (!dispatchRequest.isSuccess()) { // 校验报错，commitlog中存在异常数据
                    log.info("recover physics file end, " + mappedFile.getFileName());
                    break;
                }
            }

            // 这部分逻辑和ConsumeQueue都是一样的
            ...
            if (maxPhyOffsetOfConsumeQueue >= processOffset) { // 清空消费队列中多余的数据
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        final List<MappedFile> mappedFiles = this.mappedFileQueue.getMappedFiles();
        if (!mappedFiles.isEmpty()) {
            /* 与recoverNormally不同的是：
                1. recoverNormally是从倒数第三个文件开始恢复
                2. recoverAbnormally是从最后一个文件开始找到第一个消息正常存储的文件
             */
            ...
            while (true) {
                ...
                if (dispatchRequest.isSuccess()) {
                    ...
                    if (size > 0) {
                        ...
                        // 会再次进行转发调度，recoverNormally只进行校验，不会进行转发调度
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        // 如果开启了索引且索引是保证安全的，则存储时间要小于等于索引文件的最近刷盘时间
        if (this.defaultMessageStore.getMessageStoreConfig().isMessageIndexEnable()
            ...
            // 没有保证索引文件的安全，则只需要小于等于commitlog和consumequeue文件的最新刷盘时间
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // 如果是延迟消息，更新topic为延迟消息专用topic
                topic = TopicValidator.RMQ_SYS_SCHEDULE_TOPIC;
                // 每个不同的延迟级别对应不同的队列
                queueId = ScheduleMessageService.delayLevel2QueueId(msg.getDelayTimeLevel());

                // Backup real topic, queueId 将原始的topic和queueId保存到properties中
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        // File at the end of the minimum fixed length empty 文件最后至少需要保留8个字节空白，前4个字节用于存储剩余空间，后4个字节存储魔数
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            String msgId; // 全局唯一消息： ip+端口+消息偏移量
            if ((sysflag & MessageSysFlag.STOREHOSTADDRESS_V6_FLAG) == 0) {
            ...
            // Record ConsumeQueue information 获取该在消费队列的offset，记录到CommitLog中
            ...
            // Exceeds the maximum message 消息长度溢出
            ...
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) { // 消息长度 + 保留的空白 是否超过的当前文件的容量？
            ...
            switch (tranType) {
                ...
                case MessageSysFlag.TRANSACTION_ROLLBACK_TYPE:
                    break;
                // 如果不是事物消息或者是事务的提交消息，更新消费队列的offset
                case MessageSysFlag.TRANSACTION_NOT_TYPE:
                case MessageSysFlag.TRANSACTION_COMMIT_TYPE:
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            while (messagesByteBuff.hasRemaining()) { // 遍历每条消息
                // 1 TOTALSIZE
            ...
            CommitLog.this.topicQueueTable.put(key, queueOffset); // 更新消费队列offset
